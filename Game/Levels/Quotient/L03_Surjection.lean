...
The simplest induction principle `Quotient.ind` for the quotient type states that we can prove a
property of elements of `Quotient r` if we can prove that it holds for all typical elements `⟦a⟧`.
