...
Statement prime_dvd_prime_iff_eq {a b : ℕ} (ha : Prime a) (hb : Prime b) :
  ...
  **Robo**: Genau."
  ...
  · intro h
    ...
    Hint (hidden := true) "Try proving `Prime {b}` by `prime_def`. Leave `Prime {a}` as is."
    rw [prime_def] at hb
    ...
    · Hint "Story"
      ...
      Hint (hidden := true) (strict := true) "Try `have h' := not_prime_one`"
...
"
-/
Conclusion "Conclusion Prado O06"
