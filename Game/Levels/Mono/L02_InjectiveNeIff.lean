...
Statement (f : ℤ → ℤ) (hf : Injective f) {n : ℕ} (hn : Odd n) :
    f 1 ≠ f ((-1) ^ n) := by
  /-
  ...
  that `f a ≠ f b ↔ a ≠ b`"
  rw [Injective.ne_iff]
  Hint (hidden := true) "Perhaps, try using the theorem `Odd.neg_pow` here"
  rw [Odd.neg_pow]
  Hint (hidden := true) "Perhaps, try using `grind` here"
  grind
  · assumption
  · assumption
