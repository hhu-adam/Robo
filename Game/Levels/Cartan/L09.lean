...
Statement : ¬ (∀ᶠ x in 𝓝[≠] (0 : ℝ), 1 / x > 5) := by
  ...
  have h' : ∀ᶠ x in 𝓝[<] (0 : ℝ), 1 / x > 5 := by
    ...
    Hint (hidden := true) "[Hint hfltm] Apply this theorem by using `apply Filter.Eventually.filter_mono _ {h}`."
    apply Filter.Eventually.filter_mono _ h
  ...
  have : ∀ᶠ (x : ℝ) in 𝓝[<] 0, 1 / x > 5 ∧ x ∈ Set.Iio 0 := by
    Hint "[Hint cghfe1] For a filter `𝓕`, `p x ∧ q x` holds eventually in 𝓕 is equvalent to
  ...
  Hint (strict := true) "[Hint c9ea] For a filter `𝓕` and a proposition `p x` eventually holds in 𝓕, then
    there is a element in 𝓕, such that p x holds. This is `Filter.Eventually.exists` in Mathlib."
  have exist_aux : ∃ (x : ℝ), 1 / x > 5 ∧ x ∈ Set.Iio 0 := by
    apply Filter.Eventually.exists this
