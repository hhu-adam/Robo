...
```
lemma Fin.sum_univ_castSuccEmb {β : Type _} [AddCommMonoid β]
    {n : ℕ} (f : Fin (n + 1) → β) :
    ∑ i : Fin (n + 1), f i =
...
**Du**: Dann könnte ich also einfach `Fin.sum_univ_castSuccEmb f` schreiben?
...
Statement (m : ℕ) : ∑ i : Fin (m + 1), (i : ℕ) + (m + 1) = ∑ i : Fin (Nat.succ m + 1), ↑i := by
  Branch
    ...
    explizit mit `Fin.sum_univ_castSuccEmb (n := m + 1)` nachhelfen."
    rw [Fin.sum_univ_castSuccEmb]
    ...
    nicht mehr geht, aber das war nicht der Punkt…"
    rfl
  rw [Fin.sum_univ_castSuccEmb (n := m + 1)]
...
**Robo** `@Fin.sum_univ_castSuccEmb` würde *alle* Argumente explizit machen,
aber das ist unparktischer, weil man dann irgendwie
`@Fin.sum_univ_castSuccEmb _ _ (m + 1)` schreiben müsste.
